...
from pmsa.image.models.forensic_features import ArtifactDetector
...
def artifact_to_text(scores: Dict[str, float]) -> str:
    ...
    if scores["block_dct_energy"] > 0.06:
        explanations.append("Elevated mid-frequency DCT energy indicates block quantization or resampling artifacts.")

    if scores["residual_energy"] > 0.02:
        explanations.append("High residual energy suggests unnatural fine-detail textures.")

    if scores["checkerboard_score"] > 0.02:
        explanations.append("Checkerboard-like patterns detected, often caused by upsampling or diffusion artifacts.")
...
def explain_image(model, frame: torch.Tensor, target_layer: Optional[str] = None) -> Tuple[Optional[np.ndarray], str]:
